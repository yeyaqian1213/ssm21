  ...
  <body>
    ...
    <form action="chooseCourse/addChooseCourse.htm" method="post">
		...
		<table>
			...
			<tr>
				<td>开课学院：<select id="collegeId" name="college.id" onchange="getCourse()">
						<c:forEach items="${collegeList }" var="college">
							<option value="${college.id }" 
								id="college${college.id }" 
								collegeName="${college.name }"
								onclick="getCollegeName(this)"
								>${college.name }</option>
						</c:forEach>
				</select>
				
				<input name="college.name" id="collegeName" type="hidden">
				
				</td>
			</tr>
			<tr>
				<td>
				 	课程名称：<select id="courseSelect" name="course.id">
						<option value="0">请选择课程</option>
						</select>
						
					<input name="course.name" id="courseName" type="hidden">
					<input name="course.credit" id="courseCredit" type="hidden">
			...
			<tr>
				<td>
				 	老师名称：<select id="teacherSelect" name="teacher.id">
						<option value="0">请选择老师</option>
						</select>
						
					<input name="teacher.name" id="teacherName" type="hidden">
				</td>
			</tr>
			<tr>
				<td>
				 	上课人数：<input id="max" type="text" name="max" onchange="classroomSize()"><label id="msgSize"></label>
				</td>
				
			</tr>
			<tr>
				<td>
				 	教室及时间：<select id="classTimeSelect" name="classTime.id">
						<option value="0">请选择教室及时间安排</option>
						</select>
					
					<input name="classTime.className" id="classTimeName" type="hidden">
